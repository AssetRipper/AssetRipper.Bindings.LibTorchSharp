...
// Polyfill until .NET 11
#pragma warning disable CS3021 //Does not need CLSCompliant attribute
...
namespace System.Numerics
{
	...
	{
		private const byte floatExponentBias = 127;
		private const uint floatSignMask = 0x8000_0000;
		private const int floatBiasedExponentShift = 23;
		private const ulong doubleSignMask = 0x8000_0000_0000_0000;
		private const ulong doubleBiasedExponentMask = 0x7FF0_0000_0000_0000;

		private static float floatCreateSingle(bool sign, byte exp, uint sig) => BitConverter.UInt32BitsToSingle((sign ? floatSignMask : 0U) + ((uint)exp << floatBiasedExponentShift) + sig);
		...
		public int CompareTo(object? obj)
		{
			...
			{
				return (obj is null) ? 1 : throw new ArgumentException(null, nameof(obj));
			}
		...
		{
			return ((float)this).TryFormat(utf8Destination, out bytesWritten, format, provider);
		}
		...
		public static explicit operator BFloat16(double value)
		{
			...
			// Exponent mask
			const ulong SingleBiasedExponentMask = doubleBiasedExponentMask;
			...
			// Extract sign bit
			ulong sign = (bitValue & doubleSignMask) >> 48;
			...
			where TInteger : unmanaged, IBinaryInteger<TInteger>, ISignedNumber<TInteger>
		{
			...
			float scaleFactor = floatCreateSingle(sign, (byte)(sizeof(TInteger) * 8 - SignificandLength - scale + floatExponentBias), 0);
			...
			where TInteger : unmanaged, IBinaryInteger<TInteger>, IUnsignedNumber<TInteger>
		{
			...
			float significand = (float)int.CreateTruncating(significandBits);
			// Craft the value 2^(size-SignificandLength-scale)
			float scaleFactor = floatCreateSingle(false, (byte)(sizeof(TInteger) * 8 - SignificandLength - scale + floatExponentBias), 0);
			float roundedValue = significand * scaleFactor;
...
#pragma warning restore CS1718
		...
		{
			if (float.TryParse(utf8Text, style, provider, out float floatResult))
			{
				result = (BFloat16)floatResult;
				return true;
			}
